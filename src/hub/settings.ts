/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 Discord 配置
 */
...
<head>
  ...
  <title>Discord Bridge — 设置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Discord Bridge 设置</h1>
...
/**
 * POST /settings/save — 保存修改后的 Discord 配置
 */
export async function handleSettingsSave(
  ...
  const existingConfig = store.getConfig(installationId);
  const newConfig: Record<string, string> = { ...existingConfig };

  if (discordBotToken) newConfig.discord_bot_token = discordBotToken;
  // discord_channel_id 允许清空，所以特殊处理：如果表单中有该字段则始终更新
  newConfig.discord_channel_id = discordChannelId;

  // 加密保存
  store.saveConfig(installationId, newConfig);
...
function renderConfigForm(
  ...
  const cfg = store.getConfig(installationId);
  const maskedBotToken = maskValue(cfg.discord_bot_token || "");
  const maskedChannelId = maskValue(cfg.discord_channel_id || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Discord Bridge — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Discord Bridge 设置</h1>
    <p class="desc">修改您的 Discord 应用凭证配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 Bot Token</dt>
      <dd>${maskedBotToken || "（未配置）"}</dd>
      <dt>当前频道 ID</dt>
      <dd>${maskedChannelId || "（未配置）"}</dd>
    </div>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="discord_bot_token" type="password" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://discord.com/developers/applications" target="_blank">Developer Portal</a> → Bot → Reset Token 获取</p>

      <label>Discord 频道 ID</label>
      <input name="discord_channel_id" placeholder="留空则清除" value="${escapeHtml(cfg.discord_channel_id || "")}" />
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-discord">自行部署</a>本 App</li>
        </ul>
      </div>
