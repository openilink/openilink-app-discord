...
// 安装 ID（由 Store 自增分配，通过 setInstallationId 设置）
let installationId = "1";

/** 设置当前安装 ID（Store 自增后调用） */
 ...
 * - POST /api/bot/:botId/message/send — 记录 App 通过 HubClient 发送的消息
 * - GET /mock/messages — 返回记录的消息列表
 * - POST /mock/reset — 清空记录
 * - GET /health — 健康检查
 *
 * @param port Mock Server 监听端口
 * @param webhookUrl App 的 webhook URL（如 http://localhost:9912/hub/webhook）
 */
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/mock/event") {
      try {
        ...
        // 构造 HubEvent（Discord 的 WxToDiscord 从 eventData.sender 读取发送者信息）
        const hubEvent = {
          v: 1,
          type: "event",
        ...
        // 计算签名（Discord 版 webhook 使用 x-hub-timestamp 和 x-hub-signature）
    ...
    // POST /api/bot/:botId/message/send — 记录 App 通过 HubClient 发送的消息
    const botSendMatch = url.pathname.match(/^\/api\/bot\/[^/]+\/message\/send$/);
    if (req.method === "POST" && botSendMatch) {
